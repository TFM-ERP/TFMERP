 ...
 *
 *   1. IT WILL NOT ANCHOR AN UNSOUND LABEL. 28 of the 91 rows name more than one polity, or name
 ...
 *      calendar: it has 34 gaps of more than 25 years (41 of any size, and 2,430 unlabelled years between the UAE's
 *      Bronze Age and its Islamic era) and 2 overlaps. So era -> year only. A year -> era lookup would be undefined
...
export function anchorYearForEra(country: string, label: string, presentYear: number): AnchorVerdict {
  ...
  if (row.start === null) {
    return {
      year: null,
      status: 'refused',
      note: `"${row.label}" has no sourced start, so there is no midpoint to take. Set the year you mean.`,
    };
  }
  if (row.flag === 'unsound') {
    return {
  ...
  if (row.flag === 'disputed') {
    return { year, status: 'ok', note: `Dated ${describeRange(row, presentYear)}; that boundary is disputed. Change it if you mean otherwise.` };
  }
  return { year, status: 'ok', note: '' };
}
