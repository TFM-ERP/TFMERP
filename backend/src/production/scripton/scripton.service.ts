...
export class ScripOnService {
  ...
  async generateStage(opts: any, userId?: string) {
    ...
    if (wantsSource && rawSource && opts?.buildId) {
    ...
    const sourceFacts = wantsSource ? await this.sourceCanonFor(projectId, rawSource) : [];
  ...
  async extractCanonForBuild(buildId: string): Promise<{ state: string; facts: number }> {
    ...
    try {
      const facts = await this.sourceCanonFor(b.projectId || b.id, src);
      await set({ canonState: 'ready', canonAt: new Date(), canonError: null });
      this.log.log('canon ready for build ' + b.id + ': ' + facts.length + ' fact(s).');
      return { state: 'ready', facts: facts.length };
    } catch (e: any) {
