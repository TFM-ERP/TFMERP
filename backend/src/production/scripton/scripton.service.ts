...
export class ScripOnService {
  ...
  async generateStage(opts: any, userId?: string) {
    ...
    // THE GATE. A stage does not run without its canon, and it does not WAIT for it either — the
    // extraction is a build-level step now (see extractCanonForBuild), so this reads a state and
    // refuses. Waiting inline is what made a 34,000-token canon failure look like a failed LOGLINE.
    //
    // NULL IS LEGACY, NOT BLOCKED. The 22 builds that predate canonState have never been extracted;
    // refusing them outright would have been a migration that killed every existing build. The first
    // request starts the work and says so, and the next one proceeds.
    // 400, not merely non-empty: below that no canon is ever extracted (createBuild leaves the state
    ...
    if (wantsSource && rawSource.length >= 400 && opts?.buildId) {
      ...
      if (st === 'ready') {
        ...
        // facts it certified: a restart erased them and the next stage quietly re-extracted inline —
        // the very wait this gate exists to keep out of a stage call. Ready now means a stored row
        // (or this process's memory of one); if neither exists, extract once, store it, and say so.
        canon = await this.loadSourceCanon(projectId, rawSource, { extract: false });
        if (!canon) {
          void this.extractCanonForBuild(String(opts.buildId)).catch(() => {});
          throw new BadRequestException('This build\'s canon was only ever held in memory — it predates stored canon — '
            + 'and a restart cleared it. It is being extracted again now, once, and stored this time. Try this stage again in a minute.');
        }
      } else {
        if (!st) {
          void this.extractCanonForBuild(String(opts.buildId)).catch(() => {});
          throw new BadRequestException('This build predates the canon step, so its canon is being extracted now — '
            + 'a one-off for older builds. Try this stage again in a minute.');
        }
        if (st === 'pending' || st === 'running') {
          throw new BadRequestException('The canon for this build is still being extracted (' + st + '). '
            + 'Every stage is written against it, so generating now would write blind. Try again in a minute.');
        }
        throw new BadRequestException('The canon for this build FAILED to extract, so no stage can be written '
          + 'against it: ' + String((buildRow && buildRow.canonError) || 'no reason recorded')
    ...
    if (sourceFacts.length) {
      const account = canon && canon.account;
      if (account) {
        data.canon = account;
   ...
   *
   * STORED, KEYED ON WHAT IT WAS EXTRACTED FROM. This was an in-memory map keyed on the project, so
   * a build marked "ready" certified a canon that a restart erased and nobody could read back — the
   * ACCOUNT was persisted on every stage version, the facts it described were not. The facts now live
   * in source_canons under sha256(source) + CANON_EXTRACTOR_VERSION: a restart reads them back, and a
   * second build on the same bible (a branch, a re-run) reuses the extraction instead of paying for
   * it again. Memory is only a cache in front of that row.
   *
   * THE REGISTER IS COMPOSED ON READ. It is transcribed from the source with no model call (see
   * transcribeRegister), so it is recomputed every time the canon is loaded; bundling it into the
   * paid extraction would have made every detection fix a re-extraction. It is still written to the
   * row, so what "ready" certifies can be read back whole.
   *
   * FAIL-OPEN FOR THE EXTRACTION, NOT FOR THE REGISTER — see sourceCanonFor.
   */
  ...
  private async sourceCanonFor(projectId: string, sourceText: string): Promise<{ facts: CanonFactCore[]; account: any }> {
    ...
    try {
      ...
      return got ? { facts: got.facts, account: got.account } : { facts: [], account: null };
    } catch (e) {
      if (e instanceof BadRequestException) throw e;   // the size bound is an answer, not an outage
      // The register needs no model, so a failed extraction still leaves every transcribed rule in
      // the prompt — the part of the canon a draft most often broke.
      this.log.warn('sourceCanonFor: extraction failed - the stage carries the register only. ' + this.why(e));
      const c = this.composeCanon(src, [], { extractionFailed: String(this.why(e)).slice(0, 300) });
      return { facts: c.facts, account: c.account };
    }
  }

  /**
   * The canon for this source: memory, then the stored row, then — only when `extract` — one paid
  ...
  private async loadSourceCanon(projectId: string, sourceText: string, opts: { extract: boolean })
    ...
    const row = await this.readStoredCanon(digest);
    if (row) {
      const c = this.composeCanon(src, Array.isArray(row.facts) ? row.facts : [], row.account);
      this.sourceCanonCache.set(key, { facts: c.facts, account: c.account });
      // A register written under an older detection rule is refreshed in place — free, no model call.
      if (!row.register || row.register.version !== REGISTER_VERSION) {
        void (this.prisma as any).sourceCanon.update({ where: { id: row.id }, data: { register: c.register } })
          .catch((e: any) => this.log.warn('source canon: register refresh not stored — ' + this.why(e)));
      }
      return { facts: c.facts, account: c.account, from: 'stored' };
    }
    if (!opts.extract) return null;
    // ONE EXTRACTION PER SOURCE AT A TIME. createBuild, the legacy gate and a second tab can all ask
    // for the same bible at once, and each of those is a paid call for a document already in flight.
    const running = this.canonInFlight.get(key);
    if (running) return running;
    const p = this.extractSourceCanon(projectId, src, digest, key).finally(() => this.canonInFlight.delete(key));
    this.canonInFlight.set(key, p);
    return p;
  }

  private canonKeyOf(src: string): { digest: string; key: string } {
    const digest = sourceFingerprint(src).digest || String(src.length);
    return { digest, key: digest + ':v' + CANON_EXTRACTOR_VERSION + ':r' + REGISTER_VERSION };
  }

  /** Extracted facts first, then the register transcribed from the same source. */
  private composeCanon(src: string, extracted: CanonFactCore[], account: any) {
    const reg = transcribeRegister(src);
    // THE SAME SENTENCE TWICE IS REPORTED, NOT REMOVED. The model extracts from §29 and §31 as well,
    // and a located fact whose offset falls inside a register line restates it. Removing one would
    // need a rule for which wording the author meant, and the verbatim one always wins that anyway.
    const spans = reg.facts.map((f) => [f.sourceOffset as number, (f.sourceOffset as number) + f.statement.length]);
    const overlap = extracted.filter((f) => typeof f.sourceOffset === 'number'
      && spans.some(([a, b]) => (f.sourceOffset as number) >= a && (f.sourceOffset as number) < b)).length;
    const report = { ...reg.report, overlap };
    return {
      facts: extracted.concat(reg.facts),
      account: { ...(account || {}), register: report },
      register: { version: REGISTER_VERSION, facts: reg.facts, report },
    };
  }

  /** P2021 is Prisma's "table does not exist": the migration has not been applied yet. */
  private noCanonTable(e: any): boolean {
    return !!e && (e.code === 'P2021' || String(e.message || '').includes('source_canons'));
  }

  /** The stored row, or null when there is none — or no table yet, in which case the canon lives in
   *  memory only, exactly as it did before. Any other error is thrown, not swallowed into "none". */
  private async readStoredCanon(digest: string): Promise<any | null> {
    const d = (this.prisma as any).sourceCanon;
    if (!d || typeof d.findUnique !== 'function') return null;
    return d.findUnique({ where: { digest_extractorVersion: { digest, extractorVersion: CANON_EXTRACTOR_VERSION } } })
      .catch((e: any) => { if (this.noCanonTable(e)) return null; throw e; });
  }

  /** 'stored', or 'no-table' before the migration. Anything else throws: a paid extraction that
   *  silently failed to store is the exact defect this table exists to end. */
  private async writeStoredCanon(digest: string, src: string, facts: CanonFactCore[], account: any, register: any): Promise<'stored' | 'no-table'> {
    const d = (this.prisma as any).sourceCanon;
    if (!d || typeof d.upsert !== 'function') return 'no-table';
    const data = { sourceChars: src.length, facts, account, register };
    try {
      await d.upsert({
        where: { digest_extractorVersion: { digest, extractorVersion: CANON_EXTRACTOR_VERSION } },
        create: { digest, extractorVersion: CANON_EXTRACTOR_VERSION, ...data },
        update: data,
      });
      return 'stored';
    } catch (e) { if (this.noCanonTable(e)) return 'no-table'; throw e; }
  }

  /** One paid extraction of the whole source, located, stored, cached. Throws on failure. */
  private async extractSourceCanon(projectId: string, src: string, digest: string, key: string)
    : Promise<{ facts: CanonFactCore[]; account: any; from: 'extracted'; stored: 'stored' | 'no-table' }> {
    ...
    // source is identical. What changed is what we did with it. (See canonKeyOf; the stored row is
    // keyed on the same digest and version, and REGISTER_VERSION rides only on the memory key,
    // because the register is recomputed on read and never needs a re-extraction.)
    {
      ...
      // it does on a prose stage, because this path used to FAIL OPEN: its catch returned [], and an
      ...
      // failed, and nothing empty is stored where a later read would take it for the answer.
      if (!facts.length) {
        throw new Error('the extraction returned no facts from ' + src.length + ' characters of source — '
      ...
      const c = this.composeCanon(src, facts, account);
      // STORED BEFORE IT IS SERVED. The row holds the extracted facts and the account that describes
      // them — cached WITH the facts, so a hit never reports some other run's allocation — plus the
      // register as transcribed today, so what "ready" certifies can be read back whole.
      const stored = await this.writeStoredCanon(digest, src, facts, account, c.register);
      if (stored === 'no-table') this.log.warn('sourceCanonFor: canon NOT STORED — the source_canons table is not there yet '
        + '(apply the migration). It lives in memory until the next restart, as it did before.');
      this.sourceCanonCache.set(key, { facts: c.facts, account: c.account });
  ...
  async listBuilds(projectId?: string, bin?: boolean, view?: string) {
    ...
    const select: any = { id: true, name: true, status: true, createdAt: true, updatedAt: true, deletedAt: true, archivedAt: true,
      canonState: true, canonError: true, canonAt: true,
  ...
  private buildCard(r: any, mine: any[], ladder?: { done: number; total: number; furthest: string | null; lastWorkedAt?: Date | null; incomplete?: string[] }): any {
    {
      ...
      return {
        ...
        // The canon step, on the card. null = legacy (never extracted); the board says so rather
        // than leaving a build looking ready when no stage can be written against it.
        canon: { state: r.canonState || null, at: r.canonAt || null, error: r.canonError || null },
  ...
  async createBuild(data: any) {
    ...
    const brief = (data && data.brief) || undefined;
    const hasSource = !!(brief && typeof brief.sourceText === 'string' && brief.sourceText.trim().length >= 400);
    const row: any = await (this.prisma as any).developmentBuild.create({
      data: { name, projectId, status: 'DRAFT', brief, canonState: hasSource ? 'pending' : null },
    });
    // EXTRACTION STARTS HERE, WHERE THE WAIT IS EXPECTED AND A FAILURE IS ATTRIBUTABLE TO THE SOURCE.
    // It used to run inline in generateStage, so a 105,179-character bible made LOGLINE wait on a
    // 34,000-token call and a failed extraction looked like a failed stage. Fire and forget: the
    // build is returned immediately and canonState carries the outcome.
    if (hasSource) void this.extractCanonForBuild(row.id).catch(() => { /* state carries it */ });
    return row;
  }

  /**
   * Extract this build's canon and record the outcome on the build. The ONE place canonState moves.
   *
   * Not awaited by createBuild and never called from generateStage: the gate reads the state and
   * refuses, rather than waiting. A stage that blocks on a 34,000-token call is how a canon failure
   * came to look like a stage failure.
   */
  async extractCanonForBuild(buildId: string): Promise<{ state: string; facts: number }> {
    const b: any = await (this.prisma as any).developmentBuild.findUnique({
      where: { id: String(buildId) }, select: { id: true, projectId: true, brief: true },
    }).catch(() => null);
    if (!b) return { state: 'failed', facts: 0 };
    const src = String((b.brief && b.brief.sourceText) || '');
    const set = (data: any) => (this.prisma as any).developmentBuild.update({ where: { id: b.id }, data }).catch(() => {});
    if (src.trim().length < 400) { await set({ canonState: null, canonError: null }); return { state: 'skipped', facts: 0 }; }
    await set({ canonState: 'running', canonError: null });
    try {
      // THE THROWING LOADER, not the fail-open one: this is the step that decides "ready", and it used
      // to call sourceCanonFor, whose catch returns an empty list — so a failed extraction was marked
      // READY with zero facts. A stored row (or, before the migration, memory) is what ready means.
      const got = await this.loadSourceCanon(b.projectId || b.id, src, { extract: true });
      if (!got) throw new Error('the canon loader returned nothing');
      await set({ canonState: 'ready', canonAt: new Date(), canonError: null });
      this.log.log('canon ready for build ' + b.id + ': ' + got.facts.length + ' fact(s), ' + got.from
        + (got.stored ? ' (' + got.stored + ')' : '') + ' — ' + ((got.account && got.account.register && got.account.register.summary) || 'no register'));
      return { state: 'ready', facts: got.facts.length };
    } catch (e: any) {
      const why = this.why(e);
      await set({ canonState: 'failed', canonError: String(why).slice(0, 500) });
      this.log.warn('canon FAILED for build ' + b.id + ': ' + why);
      return { state: 'failed', facts: 0 };
    }
  }
