...
export class ScripOnService {
  ...
  private async intakeSteer(projectId: string): Promise<string> {
    ...
    if (Array.isArray(i.loreSelections) && i.loreSelections.length) {
      ...
      for (const s2 of i.loreSelections) { const w = Number(s2 && s2.weight) >= 3 ? 'spine' : 'accent'; parts.push('\u00b7 ' + ((s2 && s2.name) || (s2 && s2.slug) || 'element') + (s2 && s2.culture ? ' (' + s2.culture + ')' : '') + (s2 && s2.role ? ' \u2014 role ' + s2.role : '') + ', ' + w + '.'); }
