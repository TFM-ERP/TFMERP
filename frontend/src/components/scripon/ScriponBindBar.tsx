...
import { useScriponProject } from './useScriponProject';
import { useLocale } from '@/lib/i18n';

/**
 * Floating ScripON context pill. ScripON is STANDALONE — it develops in the hidden "ScripON Library" workspace and
...
export default function ScriponBindBar() {
  const { selectedId, reload } = useScriponProject();
  ...
  const onImport = async (file: File | null | undefined) => {
    if (!file) return;
    if (!selectedId) { flash(t('ScripON Library is still loading — try again in a moment.')); return; }
  ...
  return (
    <>
      <div style={pill} dir={dir} onClick={() => setOpen((v) => !v)} title={t('ScripON — standalone Library workspace')} aria-label={t('ScripON workspace')}>
        <span style={dot} />
        <span style={{ color: C.text, fontWeight: 500 }}>{t('ScripON Library')}</span>
        <span style={{ color: C.mut }}>{open ? '▾' : '▴'}</span>
      </div>

      {open && (
        <div style={card} dir={dir} role="dialog" aria-label={t('ScripON workspace')}>
          <div style={{ fontSize: 12, color: C.mut, marginBottom: 2 }}>{t('ScripON develops in')}</div>
          <div style={{ fontSize: 14, color: C.text, fontWeight: 600 }}>
            {t('The ScripON Library')} <span style={{ color: C.ok, fontWeight: 500, fontSize: 12 }}>· {t('standalone')}</span>
          </div>
