 ...
 *  in TFM (ScripON Library reader, Print/Download PDF, Develop, Studio). The "unify script view"
...
export function buildScriptPrintHtml(text: string, title: string, info?: any): string {
  ...
  const tp = '<section class="uvp-page uvp-tp' + arCls + '"><div class="uvp-tt">' + escHtml(title || 'Untitled') + '</div>'
    ...
    + '<div class="uvp-tf">' + escHtml(foot) + '<br>' + escHtml(i.org || (ar ? 'FilmOS · سكريبت أون — الصُنّاع' : 'FilmOS · ScripON — The Film Makers FZ LLC')) + '</div></section>';
