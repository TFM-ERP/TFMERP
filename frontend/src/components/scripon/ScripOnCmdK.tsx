'use client';
/** ScripON Doctor — Command Palette (carbon-copy of design/command-palette.html). Workspace-wide ⌘K accelerator.
...
export default function ScripOnCmdK() {
    ...
    <div className="sxk" dir={dir}>
      ...
      <div className="scrim" onClick={() => setOpen(false)}>
        <div className="palette" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="palfoot"><span><kbd>↑↓</kbd>{t('navigate')}</span><span><kbd>↵</kbd>{t('open')}</span><span><kbd>esc</kbd>{t('dismiss')}</span><span style={{ marginInlineStart: 'auto', color: '#E6D2A2' }}>{t('ScripON Doctor')}</span></div>
        </div>
      </div>
    </div>
  );
