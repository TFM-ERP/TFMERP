...
/* ── border-glow-card (slice E) — holographic + warm-gold border around the render card while a
   script renders (node 104:11). The card content/layout is unchanged; the glow wraps it.
   --border-radius reconciled to the card's own 22px. */
...
.border-glow-card {
  ...
  --border-radius: 22px; --glow-padding: 40px; --cone-spread: 25;
...
export default function ScriptOnBuildScreen({ title, items, status, error, onContinue, directions, onPick, onRegen, progress, actions }: { title: string; items: { label: string; state: 'done' | 'active' | 'wait' | 'error' }[]; status: string; error?: string | null; onContinue?: () => void; directions?: any[] | null; onPick?: (d: any) => void; onRegen?: (d: any, note: string) => Promise<any | null>; progress?: number | null; actions?: { label: string; primary?: boolean; onClick: () => void }[] | null }) {
  ...
  // Glow sweeps while the card is actively rendering/building; off when done (actions shown) or paused (error).
  const glowOn = !error && !(actions && actions.length);
  return (
    <div className="tfmbuild" dir={dir}>
      ...
      {showDirs ? (
        ...
        <div className={'border-glow-card' + (glowOn ? ' sweep-active' : '')} style={{ ['--card-bg']: 'rgba(21,24,29,.62)', ['--glow-color']: 'hsl(40 80% 80%)' } as React.CSSProperties}>
