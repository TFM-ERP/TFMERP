  ...
  const onPromoteScript = async (versionId: string) => {
    ...
    try {
      ...
      const finish = (pages: number) => {
        ...
        setBuildActions([
          { label: '\u25a4  ' + t('Read the script'), primary: true, onClick: () => router.push('/scripon/script?doc=' + docId) },
          { label: '\u2193  ' + t('Download PDF'), onClick: () => { try { window.open('/scripon/script?doc=' + docId + '&print=1', '_blank'); } catch { router.push('/scripon/script?doc=' + docId + '&print=1'); } } },
