'use client';
/** ScripON — Canon route /scripon/canon. Under the `new` shell flag it renders
...
import ScriponCanon from '@/components/scripon/canon/ScriponCanon';
import type { Fact } from '@/components/scripon/canon/scripon-canon.logic';
...
export default function ScripOnCanonPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (alive) setTitle(proj.name || proj.title || 'ScripON');
        ...
        if (alive) setTitle(doc.title || proj.name || 'ScripON');
  ...
  const onNav = (k: string) => {
    ...
    if (k === 'reader' || k === 'write') return router.push('/scripon/reader');
    if (k === 'develop') return router.push('/scripon/studio?tab=builds');
    if (k === 'doctor') return router.push('/scripon/doctor');
    if (k === 'versions') return router.push('/scripon/revisions');
    if (k === 'room') return router.push('/scripon/notes');
    if (k === 'slate' || k === 'library') return router.push('/scripon/library');
    if (k === 'studio' || k === 'settings') return router.push('/scripon/settings');
