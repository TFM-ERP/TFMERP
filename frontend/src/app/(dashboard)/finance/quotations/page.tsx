  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="marquee-panel flex items-center justify-between flex-wrap gap-3">
        <div>
          <div className="text-[9.5px] font-bold uppercase" style={{ letterSpacing: '.2em', color: 'var(--gold)' }}>{t('Finance · Sales')}</div>
          <h1 className="text-[20px] font-extrabold leading-tight" style={{ color: 'var(--text-1)' }}>{t('Quotations')}</h1>
          <p className="text-sm" style={{ color: 'var(--text-3)' }}>{total} {t('quotations total')}</p>
        </div>
        <Link href="/finance/quotations/new" className="btn-primary">
          <Plus size={15} /> {t('New Quotation')}
        </Link>
      </div>
      ...
      <div className="card p-4 flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            placeholder={t('Search by number, client, or subject...')}
            value={search}
      ...
      <div className="card overflow-hidden">
        <table className="w-full">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              <th className="table-th">{t('Quotation #')}</th>
              <th className="table-th">{t('Client')}</th>
              <th className="table-th">{t('Subject')}</th>
              <th className="table-th">{t('Activity')}</th>
              <th className="table-th">{t('Issue Date')}</th>
              <th className="table-th">{t('Valid Until')}</th>
            ...
            {loading ? (
              ...
              items.map(q => (
                <tr key={q.id} className="table-row">
                  ...
                  <td className="table-td text-end font-semibold">{formatCurrency(q.total)}</td>
                  <td className="table-td">
