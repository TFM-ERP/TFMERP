  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="marquee-panel flex items-center justify-between flex-wrap gap-3">
        <div>
          <div className="text-[9.5px] font-bold uppercase" style={{ letterSpacing: '.2em', color: 'var(--gold)' }}>{t('Finance · Approvals')}</div>
          <h1 className="text-[20px] font-extrabold leading-tight" style={{ color: 'var(--text-1)' }}>{t('Invoice Approvals')}</h1>
          <p className="text-sm" style={{ color: 'var(--text-3)' }}>{t('Multi-step approval routing for expenses & supplier invoices.')}</p>
        </div>
        <div className="flex gap-2">
          <div className="flex rounded-lg border border-gray-200 overflow-hidden text-sm">
            <button onClick={() => setView('pending')} className={cn('px-3 py-1.5', view === 'pending' ? 'bg-brand-50 text-brand-700' : 'text-gray-500')}>{t('Pending')}</button>
            <button onClick={() => setView('all')} className={cn('px-3 py-1.5', view === 'all' ? 'bg-brand-50 text-brand-700' : 'text-gray-500')}>{t('All')}</button>
          </div>
          <button onClick={load} className="btn btn-secondary p-2"><RefreshCw size={14} className={loading ? 'animate-spin' : ''} /></button>
        </div>
      </div>

      {loading ? <div className="card p-10 text-center text-gray-400 text-sm">{t('Loading…')}</div> :
        items.length === 0 ? (
          <div className="card p-12 text-center text-gray-400">
            <CheckCircle size={28} className="mx-auto mb-2 opacity-30" />
            <p className="text-sm">{view === 'pending' ? t('Nothing awaiting approval.') : t('No approval requests yet.')}</p>
          </div>
        ) : (
                ...
                <div key={req.id} className="card">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div>
                      ...
                      <div className="text-xs text-gray-400">{req.entityType} · {t('raised')} {formatDate(req.createdAt)}</div>
                    </div>
                  ...
                  {isPending && current && (
                    <div className="border-t border-gray-100 pt-3">
                      <div className="text-xs text-gray-500 mb-2 flex items-center gap-1"><Clock size={12} /> {t('Awaiting')} <b className="text-gray-700">{current.approverRole}</b></div>
                      <div className="flex gap-2">
                        <input className="input text-sm flex-1" placeholder={t('Comment (optional)')} value={comments[req.id] || ''} onChange={e => setComments(c => ({ ...c, [req.id]: e.target.value }))} />
                        <button onClick={() => act(req.id, 'approve')} disabled={acting === req.id} className="btn btn-primary text-sm"><CheckCircle size={14} className="me-1" /> {t('Approve')}</button>
                        <button onClick={() => act(req.id, 'reject')} disabled={acting === req.id} className="btn btn-secondary text-sm text-red-600"><XCircle size={14} className="me-1" /> {t('Reject')}</button>
                      </div>
                    </div>
                  )}
                </div>
