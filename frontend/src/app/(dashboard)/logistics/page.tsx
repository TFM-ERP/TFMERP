...
export default function LogisticsDashboard() {
  ...
  return (
    <div className="font-sans p-6 max-w-5xl mx-auto">
      <div className="mb-5">
        <h1 className="text-2xl font-semibold tracking-tight text-slate-900 flex items-center gap-2"><BarChart3 style={{ color: 'var(--accent)' }} /> Logistics dashboard</h1>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-[11px] text-slate-400 uppercase tracking-wide">
            <tr><th className="text-start font-medium px-4 py-2.5">Project</th><th className="text-end font-medium px-3">Housed</th><th className="text-end font-medium px-3">Nights</th><th className="text-end font-medium px-3">Movements</th><th className="text-end font-medium px-3">Arrivals</th><th className="text-end font-medium px-3">Cost</th><th className="px-3"></th></tr>
          </thead>
          <tbody>
            {rows.length === 0 ? <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-400">No logistics activity across projects yet.</td></tr> : rows.map((p: any) => (
              <tr key={p.id} className="border-t border-slate-100 hover:bg-slate-50">
                ...
                <td className="text-end px-3 text-slate-600">{p.peopleHoused}</td>
                <td className="text-end px-3 text-slate-600">{p.roomNights}</td>
                <td className="text-end px-3 text-slate-600">{p.movements}</td>
                <td className="text-end px-3 text-slate-600">{p.arrivals}</td>
                <td className="text-end px-3 text-slate-700 font-medium">{money(p.cost)}</td>
                <td className="px-3 text-end"><Link href={`/production/projects/${p.id}`} className="text-slate-400 hover:text-slate-900 inline-flex"><ChevronRight size={16} /></Link></td>
              </tr>
            ))}
          </tbody>
        </table>
